...
CAGR | `r percent(CAGR)` | `r percent(funds[funds$Ticker=="VFIAX",]$CAGR)` 
...
The calculator came back with the following recommendations:
...
Unfortunatel VTIBX, the Total International Bond Index Fund Investor shares has only been available since
...
Let's see how this portfolio would do:
...
cov.mat = cov(returns) * 252  # annualized
risk.p = sqrt(weights %*% cov.mat %*% weights)
